...
fn parity_with_php_pcov() {
    ...
    assert!(project_root.exists(), "project root {:?} not found; set PARITY_PROJECT_ROOT", project_root);

    let dockerfile = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("Dockerfile.pcov");
    assert!(dockerfile.exists(), "Dockerfile.pcov not found at {:?}", dockerfile);
...
fn docker_phpunit(project_root: &Path, output_xml: &Path) {
    ...
    let _status = Command::new("docker")
        .args(["run", "--rm", "-w", "/app"])
        .arg("-v").arg(format!("{}:/app", project_root.display()))
        .arg("pcov-rs-parity")
        // -d memory_limit=-1: PHPUnit coverage data is memory-intensive
        .args(["php", "-d", "memory_limit=-1",
               "vendor/bin/phpunit",
               "--coverage-clover"])
...
fn pcovrs_analyze(project_root: &Path, output_xml: &Path) {
    ...
    let status = Command::new(bin)
        .arg("analyze")
        .arg("--config").arg(project_root.join("phpunit.xml"))
        .arg("--format").arg("clover")
        .arg("--output").arg(output_xml)
        .status()
...
fn parse_clover(path: &Path, abs_prefix: &str) -> CovMap {
    ...
    for file_node in doc.descendants().filter(|n| n.has_tag_name("file")) {
        let Some(name) = file_node.attribute("name") else { continue };
        ...
        for ln in file_node.children().filter(|n| n.has_tag_name("line")) {
            let Some(num) = ln.attribute("num").and_then(|s| s.parse::<u32>().ok()) else { continue };
            let count = ln.attribute("count").and_then(|s| s.parse::<u32>().ok()).unwrap_or(0);
            // Keep max if duplicate (shouldn't happen, but defensive).
            lines.entry(num).and_modify(|c| *c = (*c).max(count)).or_insert(count);
        }
    }
...
fn print_parity_report(php_cov: &CovMap, rs_cov: &CovMap) {
    ...
    for (file, php_lines) in php_cov {
        ...
        for (&line_num, &php_count) in php_lines {
            let rs_count = rs_lines.and_then(|m| m.get(&line_num)).copied().unwrap_or(0);
            match (php_count > 0, rs_count > 0) {
                (true,  true)  => tp  += 1,
                (true,  false) => fnn += 1,
                (false, true)  => { fp += 1; fp_here.push(line_num); }
                (false, false) => tn  += 1,
            }
        }
    ...
    eprintln!("True  positives  (both covered):         {tp}  ({:.1}%)", pct(tp));
    eprintln!("True  negatives  (both not covered):     {tn}  ({:.1}%)", pct(tn));
    eprintln!("False negatives  (rs under-reports):     {fnn}  ({:.1}%)  ← acceptable", pct(fnn));
    eprintln!("False positives  (rs over-reports):      {fp}  ({:.1}%)  ← bugs", pct(fp));
    ...
    if !fp_by_file.is_empty() {
        ...
        for (file, lines) in fp_by_file.iter().take(10) {
            ...
            eprintln!("  {:?}  {} line(s): {:?}{}", file, lines.len(), preview, ellipsis);
        }
    } else {
