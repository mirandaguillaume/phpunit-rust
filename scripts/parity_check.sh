...
# the current phpunit-rust summary does not. When one side does not report an
...
# phpunit-rust starts emitting assertion counts, this check tightens
# automatically.
#
# Usage:
#   scripts/parity_check.sh <project-dir> [path-to-phpunit-rust-binary]
#
# The phpunit-rust binary is located, in order of preference:
#   1. the optional 2nd argument
#   2. $PHPUNIT_RUST_BIN
#   3. ./target/debug/phpunit-rust then ./target/release/phpunit-rust
#      (relative to the repo root containing this script)
#   4. `phpunit-rust` on $PATH
...
# ----------------------------------------------------------------------------
[ "$#" -ge 1 ] || die "usage: $0 <project-dir> [phpunit-rust-binary]"
...
resolve_rust_bin() {
    ...
    if [ -x "$REPO_ROOT/target/debug/phpunit-rust" ]; then
        echo "$REPO_ROOT/target/debug/phpunit-rust"; return 0
    fi
    if [ -x "$REPO_ROOT/target/release/phpunit-rust" ]; then
        echo "$REPO_ROOT/target/release/phpunit-rust"; return 0
    fi
    command -v phpunit-rust 2>/dev/null && return 0
    return 1
}

RUST_BIN="$(resolve_rust_bin "$@")" \
    || die "could not locate phpunit-rust binary (build it with: cargo build -p phpunit-rust)"
[ -x "$RUST_BIN" ] || command -v "$RUST_BIN" >/dev/null 2>&1 \
    || die "phpunit-rust binary is not executable: $RUST_BIN"
...
# vanilla "Tests: 15, Assertions: 14, ..." line and the phpunit-rust
# "Tests: 15 total, ..." line.
extract_count() {
...
# phpunit-rust
# ----------------------------------------------------------------------------
echo "==> phpunit-rust: $RUST_BIN --project $PROJECT"
...
if [ -z "$RUST_TESTS" ]; then
    echo "----- phpunit-rust output -----" >&2
    printf '%s\n' "$RUST_OUT" | tail -n 30 >&2
    echo "-------------------------------" >&2
    die "could not parse 'Tests: N' from phpunit-rust output"
fi
echo "    phpunit-rust -> Tests=$RUST_TESTS Assertions=${RUST_ASSERTIONS:-<none>}"
...
if [ "$VANILLA_TESTS" != "$RUST_TESTS" ]; then
    ...
    echo "    phpunit-rust    : Tests=$RUST_TESTS" >&2
    STATUS=1
fi
...
if [ -n "$VANILLA_ASSERTIONS" ] && [ -n "$RUST_ASSERTIONS" ]; then
    if [ "$VANILLA_ASSERTIONS" != "$RUST_ASSERTIONS" ]; then
        ...
        echo "    phpunit-rust    : Assertions=$RUST_ASSERTIONS" >&2
        STATUS=1
    fi
else
    echo "    (skipping Assertions comparison: phpunit-rust does not report an assertion count)"
fi
